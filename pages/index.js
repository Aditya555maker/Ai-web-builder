...
export default function Home() {
  ...
  if (screen === "home" && step === 4) return (
    <div style={wrap}>
      <div style={card}>
        ...
        <input value={brandName} onChange={e=>setBrandName(e.target.value)} placeholder="Jaise: Ahmed's Kitchen..." style={input}/>
        <label style={lbl}>Kuch aur add karna hai? (optional)</label>
        <textarea value={prompt} onChange={e=>setPrompt(e.target.value)} placeholder="Jaise: Blue color use karo..." rows={2} style={{...input, resize:"none"}}/>
        ...
        <div style={{background:"#08080f",border:"2px dashed #1e1e35",borderRadius:"10px",padding:"16px",marginBottom:"16px",textAlign:"center"}}>
          ...
          <button onClick={() => fileInput.current.click()} style={{padding:"10px 20px",background:"#1a1a2e",border:"1px solid #333",borderRadius:"8px",color:"#a78bfa",cursor:"pointer",fontSize:"14px"}}>
            📷 Photos Choose Karo
          </button>
          {photos.length > 0 && (
            <div style={{marginTop:"12px",display:"flex",flexWrap:"wrap",gap:"8px",justifyContent:"center"}}>
              {photos.map((p, i) => (
                <div key={i} style={{background:"#1a1a2e",borderRadius:"8px",padding:"8px",width:"80px",textAlign:"center"}}>
                  ...
                  <select value={p.type} onChange={e => setPhotoType(i, e.target.value)} style={{width:"100%",background:"#0a0a14",border:"1px solid #333",color:"#fff",fontSize:"10px",borderRadius:"4px",marginTop:"4px",padding:"2px"}}>
                    <option value="logo">Logo</option>
                    <option value="bg">Background</option>
                    <option value="gallery">Gallery</option>
                  </select>
                  <button onClick={() => removePhoto(i)} style={{marginTop:"4px",background:"#ff4444",border:"none",borderRadius:"4px",color:"#fff",fontSize:"10px",padding:"2px 6px",cursor:"pointer"}}>✕</button>
                </div>
              ))}
            </div>
          )}
        ...
        <div style={{display:"flex",gap:"10px"}}>
          ...
          <button onClick={generate} style={{...nextBtn,background:"linear-gradient(135deg,#7c3aed,#2563eb)"}}>⚡ Website Banao!</button>
        </div>
      </div>
    </div>
  );
  ...
  if (screen === "loading") return (
    <div style={{...wrap,flexDirection:"column"}}>
      ...
      <h2 style={{marginBottom:"12px"}}>{editing ? "Edit ho rahi hai..." : "Website ban rahi hai..."}</h2>
  ...
  if (screen === "error") return (
    <div style={{...wrap,flexDirection:"column"}}>
      ...
      <p style={{color:"#f87171",marginBottom:"24px",fontFamily:"monospace",fontSize:"13px",padding:"12px",background:"rgba(239,68,68,0.1)",borderRadius:"8px",maxWidth:"400px",textAlign:"center"}}>{error}</p>
    ...
    <div style={{minHeight:"100vh",background:"#08080f",color:"#fff",fontFamily:"sans-serif",padding:"16px"}}>
      <div style={{maxWidth:"1000px",margin:"0 auto"}}>
        <div style={{display:"flex",gap:"8px",marginBottom:"12px",flexWrap:"wrap"}}>
          ...
          <button onClick={() => setShowPhotoPanel(!showPhotoPanel)} style={{padding:"10px 18px",background:showPhotoPanel?"#7c3aed":"#1a1a2e",border:"1px solid #333",borderRadius:"8px",color:"#fff",fontWeight:"700",cursor:"pointer"}}>📸 Photos</button>
        ...
        {showPhotoPanel && (
          <div style={{background:"#0e0e1c",border:"1px solid #1e1e35",borderRadius:"12px",padding:"14px",marginBottom:"12px"}}>
            <div style={{fontSize:"12px",color:"#7c3aed",fontFamily:"monospace",marginBottom:"10px",letterSpacing:"1px"}}>📸 PHOTO MANAGER</div>
            <input ref={fileInput} type="file" accept="image/*" multiple onChange={handlePhotoUpload} style={{display:"none"}}/>
            <button onClick={() => fileInput.current.click()} style={{padding:"8px 16px",background:"#1a1a2e",border:"1px solid #333",borderRadius:"8px",color:"#a78bfa",cursor:"pointer",fontSize:"13px",marginBottom:"12px"}}>+ Nai Photo Upload</button>
            {photos.length === 0 ? (
              <p style={{color:"#333",fontSize:"13px",fontFamily:"monospace"}}>Koi photo nahi!</p>
            ) : (
              <div style={{display:"flex",flexWrap:"wrap",gap:"10px"}}>
                {photos.map((p, i) => (
                  <div key={i} style={{background:"#1a1a2e",borderRadius:"10px",padding:"10px",textAlign:"center",width:"100px"}}>
                    ...
                    <select value={p.type} onChange={e => setPhotoType(i, e.target.value)} style={{width:"100%",background:"#0a0a14",border:"1px solid #333",color:"#fff",fontSize:"11px",borderRadius:"4px",marginBottom:"6px",padding:"3px"}}>
                      <option value="logo">🏷️ Logo</option>
                      <option value="bg">🖼️ Background</option>
                      <option value="gallery">🖼️ Gallery</option>
                    </select>
                    <div style={{display:"flex",gap:"4px",justifyContent:"center"}}>
                      <button onClick={() => insertPhoto(p)} disabled={editing} style={{padding:"4px 8px",background:editing?"#333":"#7c3aed",border:"none",borderRadius:"5px",color:"#fff",fontSize:"11px",cursor:"pointer"}}>{editing?"...":"Add"}</button>
                      <button onClick={() => removePhoto(i)} style={{padding:"4px 8px",background:"#ff4444",border:"none",borderRadius:"5px",color:"#fff",fontSize:"11px",cursor:"pointer"}}>✕</button>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div style={{background:"#0e0e1c",border:"1px solid #1e1e35",borderRadius:"12px",padding:"14px",marginBottom:"12px"}}>
          <div style={{fontSize:"12px",color:"#7c3aed",fontFamily:"monospace",marginBottom:"8px",letterSpacing:"1px"}}>🤖 AI SE EDIT KARO</div>
          <div style={{display:"flex",gap:"8px"}}>
            <input value={editPrompt} onChange={e => setEditPrompt(e.target.value)} onKeyDown={e => { if(e.key==="Enter") editWebsite(); }} placeholder="Jaise: Header blue karo, WhatsApp button add karo..." style={{flex:1,background:"#08080f",border:"1px solid #1e1e35",borderRadius:"8px",color:"#fff",fontSize:"14px",padding:"10px 12px",outline:"none"}}/>
            <button onClick={editWebsite} disabled={editing} style={{padding:"10px 18px",background:editing?"#333":"linear-gradient(135deg,#7c3aed,#2563eb)",border:"none",borderRadius:"8px",color:"#fff",fontWeight:"700",cursor:editing?"not-allowed":"pointer",whiteSpace:"nowrap"}}>{editing ? "⏳..." : "✏️ Edit"}</button>
          </div>
          <div style={{marginTop:"8px",display:"flex",flexWrap:"wrap",gap:"6px"}}>
            {["Header color badlo","WhatsApp button add karo","Price update karo","Dark theme karo","Contact form add karo","Font badlo"].map(s => (
              <button key={s} onClick={() => setEditPrompt(s)} style={{padding:"4px 10px",background:"#1a1a2e",border:"1px solid #2a2a4a",borderRadius:"15px",color:"#666",fontSize:"11px",cursor:"pointer"}}>{s}</button>
            ))}
          </div>
        </div>
